...
## Aktuálna verzia
**LIVE = v67** (nasadené 2026-09-07, overené: live `version.json` = 67, Actions „pages build and deployment" = success). Tablety na v54+ ju aplikujú ráno o 5:00 (ranné okno) alebo hneď cez ⟳ — do **IDENTICKÉHO** App.js (redizajn je celý v `Prehlady.js` = lazy chunk, tablet ho nesťahuje; prevádzka nezmenená, overené aj v builde: `main.js` −26 B). Obsah v67: high-fidelity redizajn Prehľadov + rozšírené Uzávierky (stĺpce A–M) + **CSV export všetkých 4 typov** (Sheet-ekvivalent — detail nižšie).
