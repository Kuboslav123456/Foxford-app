...
## Aktuálna verzia
**LIVE = v76** (nasadené 2026-09-23: Prehľady — uzávierky/tržby ÚPLNE SKRYTÉ a nenačítavajú sa, prepínač `UZAVIERKY_SKRYTE` v Prehlady.js). Predtým **v75** (Prehľady — uzávierka voliteľná, `UZAVIERKA_POVINNA`; ukážka `#prehlady-demo-papier`). Predtým **v74** (2026-09-23: karta Uzávierka DOČASNE skrytá z menu tabletu, prepínač `UZAVIERKA_TAB_ENABLED` v App.js). Predtým **v73** (nasadené 2026-09-11 večer, overené naživo: version.json=73, gh-pages bez `.map`, nový kód obnovy v bundli, konzola čistá; GAS Obchodná s READ_TOKEN nasadený Jakubom a overený: bez tokenu/so zapisovacím = Unauthorized, s kódom na obnovu = záloha, zápis appky OK). História: v67 redizajn + rozšírené Uzávierky (A–M) + CSV export; v68 periodický flush SB fronty (App.js); v69 vylepšenia Prehľadov (porovnanie/priemer/Stav dát/HACCP mriežka/Health-score/custom rozsah); v70 plná šírka; v71 svetlý vzdušný sidebar + HACCP heatmapa + Novinky; **v72 = duotone ikonky namiesto emoji (nižšie)**. v69–v72 sú len `Prehlady.js` (lazy chunk) → tabletu sa nedotkli.

### v76: Prehľady — uzávierky (tržby) SKRYTÉ — NASADENÉ (2026-09-23)
Jakub: „dajme preč uzávierky aj z prehľadov, schovať". Len `Prehlady.js` (+ bump). Prepínač `UZAVIERKY_SKRYTE = true` (pod `UZAVIERKA_POVINNA`): sekcia Uzávierky odfiltrovaná zo sidebaru (zapamätaná sekcia 'uzavierky' → Prehľad), peňažné KPI (Tržby/Karty/Qerko/Hotovosť/Priemerná denná), graf tržieb, podiely platieb, najlepší/najslabší deň, stĺpec Uzávierka v „Stave dát" (manažér aj admin) a box Uzávierka v detaile dňa sa nerenderujú. **Tabuľka `uzavierky_log` sa vôbec nenačítava** (ani predošlé obdobie, ani v deme) → tržby sa nedostanú do prehliadača manažéra. Health-score beží bez kasy (56/44, text „uzávierky sú dočasne skryté"). Texty pre ročný pohľad všetkých pobočiek upravené (už nehovoria „zobrazuje len tržby"). Dáta v Supabase nedotknuté. **Návrat**: `UZAVIERKY_SKRYTE = false` (a podľa situácie `UZAVIERKA_POVINNA = true`, tablet `UZAVIERKA_TAB_ENABLED = true`).

### v75: Prehľady — uzávierka VOLITEĽNÁ (kým sa robí na papieri) — NASADENÉ (2026-09-23)
Len `Prehlady.js` (+ bump). Prepínač `UZAVIERKA_POVINNA = false` + text `POZN_PAPIER`. Bez uzávierok v období (`bezUzavierok`): KPI Tržby/Karty/Qerko/Priemerná denná ukazujú „—" s poznámkou „uzávierky sa robia na papieri" (nie 0 € ▼100 %), Hotovosť v kase má tú istú poznámku, graf/donut majú vysvetľujúci text. Health-score bez kasy: váhy 56 % úlohy / 44 % HACCP (`health.bezKasy`), riadok „Kasa — bez dát". „Stav dát": manažér – uzávierka „–" namiesto „✗", kompletnosť dňa ju nevyžaduje, legenda upravená; admin – stĺpec „Uzávierky (voliteľné)" neutrálnou farbou + poznámka. Detail dňa: „– bez uzávierky (…)" sivo. Ukážka: `#prehlady-demo-papier` (demo bez uzávierok v aktuálnom období; DEMO regex prijíma obe hash hodnoty). **Návrat**: `UZAVIERKA_POVINNA = true` spolu s `UZAVIERKA_TAB_ENABLED = true` v App.js.

### v74: Karta Uzávierka DOČASNE SKRYTÁ z menu tabletu — NASADENÉ (2026-09-23)
Jakub: „prešli sme späť na papier, obávam sa o bezpečnosť, vrátime sa k tomu neskôr". Zmena len v `App.js`: konštanta `UZAVIERKA_TAB_ENABLED = false` (pri `BRANCH_PIN`), menu `.filter(...)` vyhodí položku, poistkový `useEffect` prepne prípadný tab 'uzavierka' na 'tasks'. Formulár, dáta (`foxford-uzavierky` v localStorage, v zálohe aj v Supabase `uzavierky_log`), odosielanie aj GAS handler ostávajú nedotknuté. **Návrat**: prepnúť na `true` + bump + deploy. Dôsledok pre Prehľady: nové uzávierky neprichádzajú → KPI Tržby prázdne a „Stav dát" hlási chýbajúce uzávierky (uzávierka je tam povinný typ) — ak to bude rušiť, prepnúť uzávierku v Prehľadoch na voliteľnú.
...
- **GAS `Code.gs`**: druhá konštanta `READ_TOKEN` (len admin, NIKDY do appky/repa). `doGet` prijme LEN `READ_TOKEN` (`normToken` ignoruje medzery/pomlčky/veľkosť písmen — kód sa diktuje po telefóne). Kým je placeholder, doGet nevydá nič. `doPost` ostáva na `TOKEN`.
- **App.js `restoreFromCloud`**: pred stiahnutím `window.prompt` na kód; zrušenie = nič; `Unauthorized` → „Nesprávny kód na obnovu.“; kód sa neukladá. Denná prevádzka (zápisy, auto-záloha, obnova zo súboru) bez zmeny.
- **`.env.production`**: `GENERATE_SOURCEMAP=false` → gh-pages už nenesie `.map`. `.claude/settings.local.json` a gitlink `.claude/worktrees/…` odstránené z repa (+ .gitignore).
- **Nasadené 2026-09-11 večer** v poradí GAS (Jakub, Nová verzia) → `npm run deploy` v73. Tablety dostanú v73 ráno o 5:00. Rozhodnutia: **Nivy GAS neriešiť** (Jakub); **Obraty desktop appka je vyradená** (fungujú len Prehľady). Kód na obnovu má Jakub v správcovi hesiel, v repe ani v appke NIE JE.
- **Lekcia z testu**: dev server s reálnou pobočkou pošle po 10 s vlastnú zálohu do GAS a prepíše dnešný riadok tabletu (stalo sa, napravené re-uploadom pôvodného snapshotu). Pri lokálnych testoch používať pobočku s placeholder URL alebo stubnuť fetch.
...
- [x] **Spoľahlivé doručenie (App.js) — HOTOVÉ vo v68:** periodický + visibility flush SB fronty (viď v68 vyššie). Zlyhaný dual-write sa teraz doposiela dovtedy, kým neprejde.
- [ ] **Idempotencia (duplicity)** — ODLOŽENÉ (riešiť ak reálny problém). Detail vo v68 sekcii.
