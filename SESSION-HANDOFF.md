...
### v73: Kód na obnovu zálohy z tabuľky (READ_TOKEN) + bez source máp — PRIPRAVENÉ, NENASADENÉ
...
- **Poradie nasadenia**: (1) Jakub vloží nový GAS s vyplnenými TOKEN + READ_TOKEN (hotový súbor v Downloads) → Nová verzia; (2) `npm run deploy` v73. Medzi (1) a rannou aktualizáciou tabletov hlási tlačidlo cloud obnovy „Unauthorized“, ostatné beží. Obraty appka (Desktop) číta doGet bez tokenu → potrebuje `&token=READ_TOKEN` alebo vyradiť.
